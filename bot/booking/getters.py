...
async def get_all_masters(dialog_manager: DialogManager, **kwargs):
    """Получение списка парикмахеров."""
    masters = dialog_manager.dialog_data["masters"]
    return {
        "masters": [master.to_dict() for master in masters],
        "text_masters": f"Для бронирования найдено {len(masters)} парикмахеров.",
    }


async def get_all_services(dialog_manager: DialogManager, **kwargs):
    """Получение списка услуг."""
    services = dialog_manager.dialog_data["services"]
    return {
        "services": [service.to_dict() for service in services],
        "text_services": f"Для бронирования найдено {len(services)} услуг.",
    }
