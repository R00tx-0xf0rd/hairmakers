...
async def cmd_about(call: CallbackQuery):
    ...
    await call.message.edit_text(
        about_text, reply_markup=main_user_kb(call.from_user.id)
    )
...
async def show_my_bookings(call: CallbackQuery, session_without_commit: AsyncSession):
    ...
    user_filter = create_model("UserIDModel", user_id=(int, ...))(
    ...
    await call.message.edit_text(
        text, reply_markup=user_booking_kb(call.from_user.id, book)
    )
...
async def show_all_my_bookings(
    ...
    user_bookings = await BookingDAO(session_without_commit).get_bookings_with_details(
        call.from_user.id
    )

    if not user_bookings:
        await call.message.edit_text(
            "😔 У вас пока нет активных бронирований.", reply_markup=None
        )
        return
    ...
    for i, book in enumerate(user_bookings):
        ...
        await call.message.answer(
            message_text, reply_markup=cancel_book_kb(book.id, cancel, home_page)
        )
...
async def cancel_booking(call: CallbackQuery, session_with_commit: AsyncSession):
    ...
    await broker.publish(
        f"Пользователь отменил запись о брони с ID {book_id}", "admin_msg"
    )
...
async def delete_booking(call: CallbackQuery, session_with_commit: AsyncSession):
    ...
    await broker.publish(
        f"Пользователь удалил запись о брони с ID {book_id}", "admin_msg"
    )
...
async def start_dialog(call: CallbackQuery):
    ...
    await call.message.edit_text(
        "Выберите необходимое действие", reply_markup=main_user_kb(call.from_user.id)
    )
